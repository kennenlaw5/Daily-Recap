function snapshot31(x,y) {
  ...
  var newCount=[0,0,0,0,0,0];var newF_I=[0,0,0,0,0,0];
  var cpoCount=[0,0,0,0,0];var cpoF_I=[0,0,0,0,0];
  var usedCount=[0,0,0,0,0];var usedF_I=[0,0,0,0,0];
    ...
    if(sheet!=null){
      ...
      for(var j=0;j<sheet.getLastRow()-2;j++){
        if(range[j][0]!=""&&range[j][19]!=""){
          team=range[j][19];
          if(range[j][22].toLowerCase()=="yes"){ newCount[newCount.length - 1] ++; }
